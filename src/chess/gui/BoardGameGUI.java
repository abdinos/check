...
public class BoardGameGUI{
    ...
    protected void paintComponent(){
        ...
        System.out.println(boardGame.getBoard().entrySet().size());
        for (Map.Entry<Integer, ChessPiece> entry : boardGame.getBoard().entrySet()){
        ...
        addMouseListener(new MouseListener() {
            ...
            public void mouseClicked(MouseEvent e) {
                ...
                if(pos_x<xEnHaut || pos_x>xEnBas || pos_y<yEnhaut || pos_y>yEnBas){
                    ...
                    if((pos_y/60)>1) {
                        chessPiece = boardGame.getBoard().get((xcase+(8*ycase)));
                    }else{
                        chessPiece = boardGame.getBoard().get(((pos_x / 60) - 1));
                    }
                    if(chessPiece !=null){
                        ...
                        if(chessPiece.getPieceColor().isWhite()){
                            //TODO
                            Collection<Movement> moves =  chessPiece.findLegalMovements(boardGame,true);
                            for (Movement movement : moves){
                                System.out.println("WHITE");
                                //System.out.println(movement.getFuturePosition());
                                int future_pos = movement.getFuturePosition();
                                int future_pos_x = future_pos+1;
                                int future_pos_y = (future_pos/8)+1;
                                System.out.println("X="+future_pos_x+"- Y="+future_pos_y);
                                //TODO : repaint
                            }

                        } else if (chessPiece.getPieceColor().isBlack()) {
                            Collection<Movement> moves=  chessPiece.findLegalMovements(boardGame,true);
                            for (Movement movement : moves){
            ...
            private void test(Graphics graphics){
                graphics.drawImage(white_pawn,0,0,60,60,null);
                //BoardGameGUI.super.paintComponent(graphics);
            }
